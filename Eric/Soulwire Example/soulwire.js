...
toggle:function(){this.running?this.stop():this.start()},draw:function(){for(var e=0;e<this.settings.iterations;e++)this.step()},export:function(){window.open(this.canvas.toDataURL(),"Crystallisation")}}),h=new dat.GUI;h.add(l.settings,"minSide").min(0).max(75).name("Min Side Length"),h.add(l.settings,"minAngle").min(0).max(1.5).step(.01).name("Min Angle (rad)"),h.add(l.settings,"iterations").min(1).max(100).name("Iterations"),h.add(l.settings,"randomness").min(0).max(1).step(.01).name("Randomness"),h.add(l.settings,"opposite").min(0).max(1).step(.01).name("Opposite Sides"),h.add(l,"toggle").name("Start / Stop"),h.add(l,"reset").name("Reset Polygons"),h.add(l,"clear").name("Clear Canvas"),h.add(l,"export").name("Save"),h.close()},
